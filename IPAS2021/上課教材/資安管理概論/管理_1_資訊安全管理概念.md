...
```
機密性(Confidentiality)
confidentiality 的意思是：「機密資訊不可揭露於未經授權的主體之下」，而其主要目的就是要維持資訊的機密性。
完整性(Integrity)
integrity 的意思是：「對任何機密資訊所施行的修改運作，都必須是經過授權且無竄改情事的」
可用性(Availability)
availability 的意思是：「已經取得授權的主體可以『及時地』與『不受中斷地』讀取或使用資訊」

鑑別性(Authenticity) -證明一主體或資源之識別就是其所聲明者的特性。鑑別性適用於如使用者、程序、系統與資訊等實體
可靠度(Reliability)
始終如一預期之行為與結果的性質
...
授權（Authorization）
依照實際需求給予實體適當的權限，一般建議採最小權限，
意即僅給予實際作業所需要的權限，避免過度授權可能造成的資訊暴露或洩漏。
紀錄（Accounting）
內容項目包含量測（Measuring）、監控（Monitoring）、報告（Reporting）與紀錄檔案(Logging)，以便提供未來作為稽核（Auditing）、
計費（Billing）、分析（Analysis）與管理之用，主要精神在於收集使用者與系統之間互動的資料，並留下軌跡紀錄。
